...
public final class RFExpectedLoad implements Serializable {
	...
	private RandomForestRegressor rf;
	...
	public Pipeline buildPipeline() {
		...
		rf = new RandomForestRegressor().setLabelCol("label").setFeaturesCol("oneHotSecond");
	...
	public CrossValidatorModel train(Pipeline pipeline, Dataset<Row> data) {
		...
		ParamMap[] paramGrid = new ParamGridBuilder().addGrid(rf.numTrees(), new int[] { 30, 50, 100, 150 })
				.addGrid(rf.maxDepth(), new int[] { 2, 3, 4 })
				.addGrid(rf.subsamplingRate(), new double[] { 0.5, 0.8, 1 }).build();
